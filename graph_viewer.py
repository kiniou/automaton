...
PLOT_CONFIG = [
    {
        ...
        "title": "Réservoir",
        "plots": [
            { "data_key": "niveau", "label": "Niveau", "unit": "cm", "color": "red", "yside": "left", "ylim": [0,50]},
            { "data_key": "volume", "label": "Volume", "unit": "L", "color": "orange", "yside": "right", "ylim": [0,250]},
        ],
    },
    {
        "id": "capteurs",
        "title": "Capteurs",
        "plots": [
            { "data_key": "temperature", "label": "Température", "unit": "°C", "color": "blue", "yside": "left", "ylim": [0,60]},
            { "data_key": "humidity", "label": "Humidité", "unit": "%", "color": "green", "yside": "right", "ylim": [0,100]},
        ],
    },
]
...
class RefreshDialog(ModalScreen[Optional[int]]):
    ...
    def _validate_and_dismiss(self) -> None:
        try:
            ...
            if new_interval > 0: self.dismiss(new_interval)
            else: self.app.bell()
        except (ValueError, TypeError): self.app.bell()

    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "ok-button": self._validate_and_dismiss()
        elif event.button.id == "cancel-button": self.dismiss()
...
class LogGraphApp(App[None]):
    ...
    def _prepare_aggregated_data(self, raw_data: Dict[str, Any], plot_width: int) -> Dict[str, Any]:
        ...
        for name, (times, values) in datasets_to_process.items():
            agg_times, agg_values = self.aggregate_data_median(times, values, self.time_window_seconds, plot_width)
            aggregated_results[name] = {"times": agg_times, "values": agg_values}
    ...
    def update_indicator_pane(self, parsed_data: Optional[Dict[str, Any]]) -> None:
        ...
        if parsed_data:
            ...
            for key in all_data_keys:
                source_data = parsed_data["serial_data"] if "niveau" in key or "volume" in key else parsed_data["gpio_data"]
                last_value = next((v for v in reversed(source_data.get(key, [])) if v is not None), None)
                last_values[key.split('_')[0]] = last_value

        self.query_one("#indicator_niveau", Indicator).update_value(last_values.get("niveau"))
        self.query_one("#indicator_volume", Indicator).update_value(last_values.get("volume"))
        self.query_one("#indicator_temperature", Indicator).update_value(last_values.get("temperature"))
        self.query_one("#indicator_humidity", Indicator).update_value(last_values.get("humidity"))
    ...
    def update_graph_panes(self, parsed_data: Optional[Dict[str, Any]]) -> None:
        ...
        for config in PLOT_CONFIG:
            ...
            tick_positions, tick_labels = self._generate_time_ticks(start_time, self.time_window_seconds)
            plot_widget.plt.xticks(tick_positions, tick_labels)
            
            for plot_info in config["plots"]:
                data = aggregated_data[plot_info["data_key"]]
                if data["times"]:
                    plot_widget.plt.ylim(*plot_info.get("ylim", [0, None]), yside=plot_info["yside"])
    ...
    def aggregate_data_median(self, times: List[float], values: List[float], time_window_seconds: int, plot_width: int) -> Tuple[List[float], List[float]]:
        if not times or not values or plot_width == 0: return [], []
        seconds_per_cell = time_window_seconds / plot_width
        bins: Dict[int, List[float]] = {}
        for time_point, value_point in zip(times, values):
            if value_point is None: continue
        ...
        for bin_index, bin_values in sorted(bins.items()):
            if not bin_values: continue
            sorted_bin = sorted(bin_values)
            n = len(sorted_bin)
            mid = n // 2
            if n % 2 == 0: median = (sorted_bin[mid - 1] + sorted_bin[mid]) / 2
            else: median = sorted_bin[mid]
    ...
    def _generate_time_ticks(self, start_time: datetime.datetime, window_seconds: int, num_ticks: int = 5) -> Tuple[List[float], List[str]]:
    ...
    def watch_refresh_interval_seconds(self, old_val: int, new_val: int) -> None:
        ...
        if self.refresh_timer is not None: self.refresh_timer.stop()
        if new_val > 0: self.refresh_timer = self.set_interval(new_val, self._live_update)

    def watch_follow_mode(self, new_val: bool) -> None:
        self.log(f"Follow mode set to {new_val}")
        if new_val: self.action_snap_to_now()
    ...
    def on_worker_state_changed(self, event: Worker.StateChanged) -> None:
        if event.worker != self.read_worker: return
            ...
            with sqlite3.connect(f"file:{DB_PATH}?mode=ro", uri=True) as conn:
                ...
                for timestamp_str, log_type, json_str in results:
                    ...
                    if log_type == "Série JSON":
                        ...
                        for key in serial_data: serial_data[key].append(data.get(key))
                    elif log_type == "GPIO JSON":
                        gpio_times.append(total_seconds)
                        for key in gpio_data: gpio_data[key].append(data.get(key))
    ...
    def action_toggle_follow_mode(self) -> None: self.follow_mode = not self.follow_mode
    def action_toggle_grid(self) -> None: self.show_grid = not self.show_grid
    ...
    def action_change_day_forward(self) -> None:
        ...
        if new_date <= datetime.date.today():
            ...
            if self.current_date == datetime.date.today(): self.action_snap_to_now()
            else:
                self.follow_mode = False
                self.current_end_time = datetime.datetime.combine(self.current_date, datetime.time.max)
                self.run_worker_safely()
    ...
    def action_move_forward(self) -> None:
        ...
        if new_end_time >= datetime.datetime.now(): self.action_snap_to_now()
        else:
            self.follow_mode = False
